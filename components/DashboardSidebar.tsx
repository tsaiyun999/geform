...
export default function DashboardSidebar({ currentYear, onYearChange }: SidebarProps) {
  ...
  return (
    // 💡 修改點：把 w-64, h-screen, sticky 都拿掉，改成 w-full h-full，讓 layout.tsx 去決定它的大小
    <aside className="w-full h-full bg-[#FEFDFB] flex flex-col text-[#604D53]">
      {/* 📍 1. 標題區：櫻花粉裝飾 + 可可棕文字 */}
      <div className="p-6 border-b border-[#F1E4E8] bg-white hidden md:block">
        {/* 💡 修改點：因為手機版的頂部導覽列已經有標題了，所以這裡加上 hidden md:block，在手機上隱藏這個重複的標題 */}
