...
export async function POST(request: Request) {
  try {
    ...
    // 3. 設定信件內容 (已修正變數對應)
    const mailOptions = {
      from: `"聯大通識中心" <${process.env.GMAIL_USER}>`, 
      to: email,                            
      subject: `【申請成功】${course || '開課'} 申請收執聯`,
      html: `
        <div style="font-family: sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
          <h2 style="color: #2c3e50;">${teacher || '老師'} 您好：</h2>
          <p>系統已收到您在 <strong>${year || '本學期'}</strong> 的開課申請，資料已存入雲端資料庫。</p>
          <hr style="border: 0; border-top: 1px solid #eee;" />
          <ul style="list-style: none; padding-left: 0;">
            <li><strong>📚 課程名稱：</strong> ${course}</li>
            <li><strong>⏳ 申請狀態：</strong> 審核中</li>
          </ul>
          <p style="margin-top: 20px;">若發現資料有誤或需調整時間，請點擊下方專屬連結進行修正：</p>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/edit/${id}" 
